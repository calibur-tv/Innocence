...
//        videoView = (IjkVideoView) activity.findViewById(R.id.video_view);
        ...
        mMediaController = videoView.getMediaController();
...
//        videoView.seekTo((int) delta);

        int showDelta = (int) delta / 1000;
        if (showDelta != 0) {
            String text = showDelta > 0 ? ("+" + showDelta) : "" + showDelta;
            LogUtils.d("ijkPlayer","onProgressSlide:" + text);
//            videoView.seekTo(showDelta);
        }
    }
...
//            videoView.toggleAspectRatio();
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            ...
            if (toSeek) {
                LogUtils.d("ijkPlayer","seek");
                if (!isLive) {
                    LogUtils.d("ijkPlayer","调节进度");
